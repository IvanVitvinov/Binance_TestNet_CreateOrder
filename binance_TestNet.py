...
class BinanceTrader:
    ...
    def create_orders(self, data, symbol):
        ...
        if data['side'] == 'SELL' and required_coin > coin_balance:
            print(f"Недостаточный баланс для создания ордеров: необходимо {required_coin} монет, имеется {coin_balance} монет")
        elif data['side'] == 'BUY' and required_usdt > usdt_balance:
            print(f"Недостаточный баланс для создания ордеров: необходимо {required_usdt} монет, имеется {usdt_balance} монет")
        else:
